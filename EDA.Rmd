...
```{r, load_time_series_data, warning=FALSE, error = FALSE}
library(extrafont)
...
```{r turnout, message=FALSE}
library(here)

turnout_2016<-read.csv(here("data", "2016.csv"))
turnout_2020<-read.csv(here("data", "2020.csv"))
turnout_2016<-turnout_2016[3:54, ]
turnout_2020<-turnout_2020[4:54, ]
```

```{r datamanip, message=FALSE}
data1<-data
turnout_2016_2020<-left_join(turnout_2016, turnout_2020, by = "ï..") %>% 
  rename(state_name= "ï..") %>% 
  select(-c(X.1.x, X.1.y)) %>% 
...
turnout_and_covid <- states %>% 
  ...
  left_join(turnout_2016_2020, by= "state_name")

```

```{r correlation_creation, message=FALSE}
library(showtext)
font_add_google("Courier Prime", "Courier")
showtext_auto()
...
correlation <-as.data.frame(cor(circles_data_sf[, 10:54], method = "pearson", use = "complete.obs")) %>% 
  mutate(names = rownames(.)) %>% 
  arrange(desc(percentage20_joe_biden)) %>% 
  filter(!names %in% c("votes16_donald_trump","votes16_hillary_clinton","percentage20_donald_trump","total_votes20", "total_votes16","votes20_donald_trump","percentage20_joe_biden", "voting_age_citizen", "votes20_joe_biden","percentage16_hillary_clinton")) %>% 
...
```{r plot corrr, message=FALSE}
cor1<-ggplot(correlation1, aes(x=reorder(names, no), y=percentage20_joe_biden, label=round(percentage20_joe_biden,2))) + 
  geom_point(stat='identity', aes(col=percentage20_joe_biden), size=9)  +
  scale_color_gradient2("",low = muted("#CB454A"),mid = "white", high = muted("#2E74C0"), midpoint=0, breaks=c(-0.5,0,0.45), labels=c("More Trump",0,"More Biden"))+ 
  ...
  labs(title="Blacks & Asians Voted J.Biden, People Working in Construction or Production Did Not", subtitle= "Correlation between Demographics and Votes for J.Biden", x="Ethnicity",y= "",fill="")+
  theme(plot.title = element_text(size=13), 
        plot.subtitle=element_text(size=12.5), 
        plot.caption = element_text(size=6))+
  coord_flip()

cor2<-ggplot(correlation2, aes(x=reorder(names, no), y=percentage20_joe_biden, label=round(percentage20_joe_biden,2))) + 
  geom_point(stat='identity', aes(col=percentage20_joe_biden), size=9)  +
  scale_color_gradient2("",low = muted("#CB454A"),mid = "white", high = muted("#2E74C0"), midpoint=0, breaks=c(-0.5,0,0.45), labels=c("More Trump",0,"More Biden"))+ 
  geom_text(color="black", size=2) +
  ylim(-0.6, 0.6) +
...
cor3<-ggplot(correlation3, aes(x=reorder(names, no), y=percentage20_joe_biden, label=round(percentage20_joe_biden,2))) + 
  geom_point(stat='identity', aes(col=percentage20_joe_biden), size=9)  +
  scale_color_gradient2("",low = muted("#CB454A"),mid = "white", high = muted("#2E74C0"), midpoint=0, breaks=c(-0.5,0,0.45), labels=c("More Trump",0,"More Biden"))+ 
  geom_text(color="black", size=2) +
  ylim(-0.6, 0.6) +
...
library(ggpubr)
library(gridExtra)
#grid.arrange(cor1, cor2, cor3, nrow = 3)
...
library(gghighlight)
tcov_points%>%
  mutate(hey=ifelse(cases_per_1M>45000,1,0)) %>% 
  ggplot(aes(x=cases_per_1M,y= X.y)) + 
  geom_point(aes(label=state_abbv))+
  geom_smooth(method= "lm", se=FALSE, color=muted("#CB454A"))+
  #geom_segment(mapping=aes(x=10000, y=77, xend=40000, yend=57), arrow=arrow(), size=1, color=muted("#CB454A"))+
  theme_minimal()+
  #geom_text_repel(aes(label=state_abbv))+
  labs(x="Coronavirus Cases per 1M", y= "Turnout Rate per State", title= "More Covid less Votes!", subtitle="States' Turnout Rates and Level of Covid per 1 Million People")+
...
statebins(state_data = turnout_and_covid, state_col = "state_name",
                  value_col = "difference",font_size = 3)+
  scale_fill_continuous(trans = 'reverse')+
   theme(panel.grid.major = element_blank(), 
         ...
         axis.ticks=element_blank())+
  theme_void()+
  labs(title="Improvement in turnout Rate 2016-2020")
```
